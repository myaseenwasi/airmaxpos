                ...
                if (storedSessionId && data.xSessionId === storedSessionId) {
                    ...
                    if (data.xGatewayStatus === 'Approved') {
                        ...
                        if ($('#modal_payment').is(':visible')) {
                            ...
                            // First payment method - set payment method to Cardknox and submit
                            $('.payment_types_dropdown').val('custom_pay_1').trigger('change');
                            
                            // Add a small delay to ensure the payment method is set
                            setTimeout(function() {
                                pos_form_obj.submit();
                            }, 100);
                        }
...
$(document).ready(function() {
            ...
            .autocomplete({
                ...
                response: function(event, ui) {
                    if (ui.content.length == 1) {
                        ...
                        window.last_searched_sku = $('#search_product').val();
                        $('input#search_product').select();
                        $('button.pos_add_quick_product').first().trigger('click');
                    }
                },
    ...
    $('#modal_payment').on('input keypress','input.payment-unit-price', function(e) {
    ...
    $('.add-payment-custom').on('click', function(e) {
        var paymentType = $('.payment_types_dropdown').val();
        var amount = $(this).data('value');
        if(paymentType == 'cash') {
            const $this = $('input.payment-amount').val(amount);
...
// on click sub category in category drawer
    ...
    $(document).on('click', 'button.pos_add_quick_product', function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                // After modal is shown, set the SKU field if last_searched_sku is set
                $(container).off('shown.bs.modal.setSku').on('shown.bs.modal.setSku', function() {
                    if (window.last_searched_sku) {
                        $('#sku').val(window.last_searched_sku);
                        window.last_searched_sku = '';
                    }
                });
            },
    ...
    $(document).on('quickProductAdded', function(e) {
        ...
        if ($('input#location_id').val() == '') {
            ...
            if (e.variation && e.variation.id) {
                pos_product_row(e.variation.id);
            } else {
                // No variation id: simulate adding a product row from the quick add form
                var form = $("#quick_add_product_form");
                var product_name = form.find('input[name="name"]').val() || 'Custom Product';
                var sku = form.find('input[name="sku"]').val() || '';
                var unit_id = form.find('select[name="unit_id"]').val() || '';
                var unit_text = form.find('select[name="unit_id"] option:selected').text() || '';
                var price = form.find('input[name="single_dsp"]').val() || 0;
                var row_count = $('input#product_row_count').val() || 0;
                var quantity = 1;
                var subtotal = parseFloat(price) * quantity;
                // Build a row matching the standard layout
                // 1. Unit as <select>
                var unit_select = '<select name="products[' + row_count + '][product_unit_id]" class="form-control">';
                form.find('select[name="unit_id"] option').each(function() {
                    var opt_val = $(this).val();
                    var opt_text = $(this).text();
                    var selected = (opt_val == unit_id) ? 'selected' : '';
                    unit_select += '<option value="' + opt_val + '" ' + selected + '>' + opt_text + '</option>';
                });
                unit_select += '</select>';
                // 2. Subtotal with $ sign, no input field, but add hidden input for pos_unit_price_inc_tax
                var row_html = '<tr class="product_row pos_table_custom_row" data-row_index="' + row_count + '">' +
                    '<td>' + product_name + '<br/>' + sku + '</td>' +
                    '<td>' +
                        '<input type="hidden" name="products[' + row_count + '][product_id]" class="form-control product_id" value="custom">' +
                        '<input type="hidden" name="products[' + row_count + '][variation_id]" class="row_variation_id" value="custom_' + row_count + '">' +
                        '<input type="hidden" name="products[' + row_count + '][enable_stock]" value="0">' +
                        '<div class="input-group input-number">' +
                            ...
                            '<input type="text" data-min="1" style="width: auto" class="form-control pos_quantity input_number mousetrap input_quantity" value="' + quantity + '" name="products[' + row_count + '][quantity]" data-allow-overselling="true" data-decimal="1" data-rule-required="true" data-msg-required="This field is required">' +
                        ...
                        unit_select +
                    '</td>' +
                    // Add hidden input for pos_unit_price_inc_tax
                    '<input type="hidden" name="products[' + row_count + '][unit_price_inc_tax]" class="form-control pos_unit_price_inc_tax input_number" value="' + price + '">' +
                    // Add hidden input for pos_line_total
                    '<input type="hidden" name="products[' + row_count + '][pos_line_total]" class="form-control pos_line_total input_number" value="' + subtotal + '">' +
                    '<td class="text-center">' +
                        '<span class="display_currency pos_line_total_text" data-currency_symbol="true">$' + subtotal + '</span>' +
                    '</td>' +
                    // Second last column: X button
                    '<td class="text-center v-center">' +
                        '<i class="fa fa-times text-danger pos_remove_row cursor-pointer" aria-hidden="true"></i>' +
                    '</td>' +
                    // Last column: empty
                    '<td class="text-center v-center"></td>' +
                '</tr>';
                $('table#pos_table tbody').append(row_html);
                // Increment product row count
                $('input#product_row_count').val(parseInt(row_count) + 1);
...
window.addEventListener("afterprint", (event) => {
    ...
    // Reset the form data in localStorage after printing
});
    ...
    $.ajax({
        ...
        success: (result) => {
            if (result.success && result.data) {
                ...
                $('#return_payment_modal #tax_span').text(__currency_trans_from_en(result.data.tax_amount,true) || '0');
                $('#return_payment_modal #tax_label_span').text(result.data.tax_label);
