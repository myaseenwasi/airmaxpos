...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction->created_by) {
            ...
            if ($user) {
                $output['user_name'] = $user->user_full_name ?? $user->first_name . ' ' . $user->last_name;
                $output['user_email'] = $user->email;
            }
        }
        
        // Business location
        $output['business_location'] = $location_details->name ?? '';
        
        // Device information - check if there's a cash register associated with this transaction
        $cash_register = \App\CashRegister::where('user_id', $transaction->created_by)
            ->where('location_id', $location_id)
            ->where('status', 'open')
            ->first();
            
        if ($cash_register && $cash_register->device_id) {
            $device = \App\Device::find($cash_register->device_id);
            if ($device) {
                $output['device_id'] = $device->device_id ?? 'Not Assigned';
                $output['station_name'] = $device->device_friendly_name ?? 'Not Assigned';
            }
        }
