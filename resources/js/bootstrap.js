...
if (typeof APP != 'undefined' && APP.PUSHER_ENABLED) {
    ...
    // Listen for Cardknox webhooks on the public channel
    window.Echo.channel('cardknox-webhooks')
        .listen('.cardknox.webhook', (e) => {
            console.log('Cardknox webhook received via Echo:', e);
            if (typeof handleCardknoxWebhook === 'function') {
                handleCardknoxWebhook(e.payload);
            } else {
                console.error('handleCardknoxWebhook function not found');
            }
        })
        .error((error) => {
